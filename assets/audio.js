...
function setBtn(on){btn.setAttribute('aria-pressed',on?'true':'false');btn.setAttribute('aria-label',on?'Pausar audio':'Reproducir audio');btn.classList.toggle('ring-2',on);btn.classList.toggle('ring-white/60',on);const ic=btn.querySelector('[data-audio-icon]');const lb=btn.querySelector('[data-audio-label]');if(ic)ic.textContent=on?'volume_up':'music_off';if(lb)lb.textContent=on?'Audio reproduciendo':'Audio apagado';} 
...
function doPlay(){if(!audio.src){audio.src=audio.dataset.src||'assets/audio/Ober-tardenoche.mp3';}audio.muted=false;try{audio.currentTime=0;}catch{}audio.play().then(()=>{fade(0.8,600);enabled=true;localStorage.setItem('bgAudio','on');setBtn(true);}).catch(()=>{});} 
...
function autoStart(){
  if(!audio.src){audio.src=audio.dataset.src||'assets/audio/Ober-tardenoche.mp3';}
