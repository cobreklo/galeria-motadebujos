...
initRipple();
function initHeroRandom(){const hero=document.getElementById('hero');if(!hero)return;const bg=hero.querySelector('.hero-media .bg-cover');if(!bg)return;function pick(){const list=window.artworks||[];if(!Array.isArray(list)||list.length===0)return;const valid=list.filter(a=>a&&a.src&&!String(a.id).startsWith('manual-'));if(!valid.length)return;const i=Math.floor(Math.random()*valid.length);const url=valid[i].src;bg.style.backgroundImage=`url("${url}")`;}window.addEventListener('artworks:ready',pick);} 
initHeroRandom();
