...
initRipple();
function initHeroRandom(){const hero=document.getElementById('hero');if(!hero)return;const bg=hero.querySelector('.hero-media .bg-cover');if(!bg)return;function pick(){const list=window.artworks||[];if(!Array.isArray(list)||list.length===0)return;const valid=list.filter(a=>a&&a.src);if(!valid.length)return;const i=Math.floor(Math.random()*valid.length);const url=valid[i].src;bg.style.backgroundImage=`url("${url}")`;}if(window.artworks&&window.artworks.length)pick();window.addEventListener('artworks:ready',pick);} 
initHeroRandom();
...
initClickBurst();
function initLogoEasterEgg(){const el=document.getElementById('siteLogo');if(!el)return;let a=new Audio(el.dataset.sound||'assets/audio/wowasound.mp3');a.preload='auto';a.volume=0.9;const play=()=>{try{a.currentTime=0;a.play();}catch{}};el.addEventListener('click',play);el.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){play();}});} 
